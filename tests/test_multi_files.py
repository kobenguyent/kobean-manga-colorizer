  ...
  <rootfiles>
    <rootfile full-path="OEBPS/content.opf" media-type="application/oebps-package+xml"/>
        ...
        for i in range(num_pages):
            ...
            manifest.append(
                f'<item id="img_{i + 1}" href="images/{img_name}" media-type="image/png"/>'
            )

            xhtml = f'<html><body><img src="images/{img_name}"/></body></html>'
            z.writestr(f"OEBPS/page_{i + 1}.xhtml", xhtml)
            manifest.append(
                f'<item id="page_{i + 1}" href="page_{i + 1}.xhtml" media-type="application/xhtml+xml"/>'
            )
...
def run_tests():
    ...
    assert len(data["sessions"]) == 2, (
        f"Expected 2 sessions in summary, got {len(data['sessions'])}"
    )
    ...
    export_resp = requests.post(
        f"{SERVER_URL}/api/export/batch", json={"session_ids": session_ids, "format": "auto"}
    )
    ...
    epub_export_resp = requests.post(
        f"{SERVER_URL}/api/export/batch", json={"session_ids": session_ids, "format": "epub"}
    )
    ...
    pdf_export_resp = requests.post(
        f"{SERVER_URL}/api/export/batch", json={"session_ids": session_ids, "format": "pdf"}
    )
