...
def create_test_pdf(filepath: str, num_pages: int = 6):
    ...
    for i in range(num_pages):
        ...
        cv2.putText(
            img, f"MODEL TEST PAGE {i + 1}", (60, 200), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 0), 2
        )
...
def run_test():
    ...
    with open(sample_pdf, "rb") as f:
        resp = requests.post(
            f"{server_url}/api/upload", files={"file": ("switch_test.pdf", f, "application/pdf")}
        )
    ...
    for _ in range(30):
        ...
        print(
            f"Status: {sess_info['status']} | Model: {sess_info.get('model_provider')} | Processed: {sess_info['processed_count']}/6"
        )
