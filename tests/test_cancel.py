...
def run_cancel_test():
    ...
    with open(sample_pdf, "rb") as f:
        resp = requests.post(
            f"{server_url}/api/upload", files={"file": ("cancel_test.pdf", f, "application/pdf")}
        )
    ...
    requests.post(
        ...
        json={"session_id": session_id, "model_provider": "local_smart"},
    )
    ...
    print(
        f"Final session status: {sess_info['status']}, Processed count: {sess_info['processed_count']}/10"
    )

    assert sess_info["status"] == "cancelled", (
        f"Expected cancelled status, got {sess_info['status']}"
    )
    assert sess_info["processed_count"] < 10, (
        "Task should have stopped before colorizing all 10 pages"
    )
