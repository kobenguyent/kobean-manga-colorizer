...
def create_sample_manga_epub(filepath: str):
    ...
    with zipfile.ZipFile(filepath, "w") as zf:
        ...
        zf.writestr(
            "OEBPS/content.opf",
            '<?xml version="1.0"?><package xmlns="http://www.idpf.org/2007/opf" version="3.0"><metadata><title>Sample Manga</title></metadata><manifest><item id="img1" href="images/page_01.png" media-type="image/png"/><item id="img2" href="images/page_02.png" media-type="image/png"/></manifest></package>',
        )
...
def run_epub_test():
    ...
    with open(sample_epub, "rb") as f:
        resp = requests.post(
            ...
            files={"file": ("sample_manga.epub", f, "application/epub+zip")},
        )
    ...
    # 2. Colorize
    requests.post(
        f"{server_url}/api/colorize/start",
        json={"session_id": session_id, "model_provider": "local_smart", "style": "anime_pastel"},
    )
