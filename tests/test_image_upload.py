...
def run_test():
    ...
    with open(sample_png, "rb") as f:
        resp = requests.post(
            f"{server_url}/api/upload", files={"file": ("sample_manga_page.png", f, "image/png")}
        )
    assert resp.status_code == 200, f"Image upload failed: {resp.text}"
    session_data = resp.json()
    session_id = session_data["session_id"]
    print(
        f"Uploaded PNG successfully! Session ID: {session_id}, Total Pages: {session_data['total_pages']}"
    )

    # 2. Preview with ResNeXt Generator
    prev_resp = requests.post(
        f"{server_url}/api/colorize/preview",
        json={"session_id": session_id, "page_index": 0, "model_provider": "resnext_generator"},
    )
