...
def create_preview_test_pdf(filepath: str):
    ...
    for i in range(4):
        ...
        cv2.putText(img, f'PREVIEW PAGE {i+1}', (80, 180), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 0), 2)
...
def run_preview_test():
    ...
    with open(sample_pdf, "rb") as f:
        resp = requests.post(f"{server_url}/api/upload", files={"file": ("preview_manga.pdf", f, "application/pdf")})
    ...
    prev_resp1 = requests.post(f"{server_url}/api/colorize/preview", json={
    ...
    prev_resp2 = requests.post(f"{server_url}/api/colorize/preview", json={
        "session_id": session_id,
        "page_index": 2,
